...
(function (root, factory) {
    ...
    Ssi_upload.prototype.init = function (element) {
        ...
        if (!this.options.preview) {
            ...
            $uploadBox = $('<div id="ssi-previewBox" class="ssi-uploadBox ssi-previewBox ' + (this.options.dropZone ? 'ssi-dropZonePreview ssi-dropZone' : '') + '"><div id="ssi-info">' + (this.options.dropZone ? '<div id="ssi-DropZoneBack">' + '드래그 하여 파일 선택하기' + '</div>' : '') + '<div id="ssi-fileNumber" class="ssi-hidden">?</div></div></div>');
            ...
            if (thisS.options.preview) {
                var getTemplate = function (content) {
                    return '<table class="ssi-imgToUploadTable ssi-pending">' +
                        ...
                        '<tr><td><button data-delete="' + index + '" class=" ssi-button error ssi-removeBtn">업로드 취소</button></td></tr>' +
    ...
    var getResultMessage = function (type, msg, title, classes) {//return a message label
        return '<span class="ssi-statusLabel  ' + classes + ' ' + type + '" data-status="' + title + '" style="font-size:8px;">' + "성공" + '</span>';
    };
